...
export default function TopFilter({ title, onOpenMenu }: Props) {
  const [showFilter, setShowFilter] = useState(false);
  const [showViewOption, setShowViewOption] = useState(false);

  // Use Zustand store for issues count
      ...
      <div className="flex justify-between flex-shrink-0 pl-2 pr-6 border-b border-gray-200 h-14 lg:pl-9">
        <div className="flex items-center">
          <button
            ...
            onClick={onOpenMenu}
          >
            <img src={MenuIcon} className="w-3.5 h-3.5 text-gray-500 hover:text-gray-800" alt="menu" />
          </button>
